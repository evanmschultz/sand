...
func TestResolveBackendsConfig(t *testing.T) {
	...
	t.Run("project_wins", func(t *testing.T) {
		...
		projectPath := writeFile(t, filepath.Join(projectDir, ".claude", "sand-backends.toml"), "project")
		writeFile(t, filepath.Join(xdgDir, "sand", "backends.toml"), "xdg")
		writeFile(t, filepath.Join(homeDir, ".config", "sand", "backends.toml"), "home-config")
		writeFile(t, filepath.Join(homeDir, ".sand", "backends.toml"), "home-dotfile")

		t.Setenv("XDG_CONFIG_HOME", xdgDir)
		t.Setenv("HOME", homeDir)

		gotPath, gotSource, err := ResolveBackendsConfig(projectDir)
		if err != nil {
			t.Fatalf("ResolveBackendsConfig: unexpected error: %v", err)
		}
	...
	t.Run("xdg_wins_when_project_absent", func(t *testing.T) {
		...
		xdgPath := writeFile(t, filepath.Join(xdgDir, "sand", "backends.toml"), "xdg")
		writeFile(t, filepath.Join(homeDir, ".config", "sand", "backends.toml"), "home-config")
		writeFile(t, filepath.Join(homeDir, ".sand", "backends.toml"), "home-dotfile")
	...
	t.Run("home_config_wins_when_project_and_xdg_absent", func(t *testing.T) {
		...
		homeConfigPath := writeFile(t, filepath.Join(homeDir, ".config", "sand", "backends.toml"), "home-config")
		writeFile(t, filepath.Join(homeDir, ".sand", "backends.toml"), "home-dotfile")
	...
	t.Run("home_dotfile_wins_when_higher_rungs_absent", func(t *testing.T) {
		...
		dotfilePath := writeFile(t, filepath.Join(homeDir, ".sand", "backends.toml"), "home-dotfile")
	...
	t.Run("xdg_unset_skips_rung_entirely", func(t *testing.T) {
		// Pinned test mirroring chains.Resolve's XDG-unset attack:
		// when XDG_CONFIG_HOME is empty, the resolver MUST NOT probe a
		// relative `sand/backends.toml` path (i.e. must not call
		// filepath.Join("", "sand", "backends.toml")). We prove this by
		// creating a deceptive `sand/backends.toml` file inside a temp
		// dir, chdir'ing into it, ensuring HOME points elsewhere with
		// no backend configs, and asserting ResolveBackendsConfig
		// returns NotFound — never the relative-path file.
		decoyDir := t.TempDir()
		decoyRelativePath := filepath.Join(decoyDir, "sand", "backends.toml")
		writeFile(t, decoyRelativePath, "decoy-must-not-match")

		// chdir so a relative "sand/backends.toml" probe would resolve
		// to the decoy file.
		cwd, err := os.Getwd()
		if err != nil {
		...
		projectDir := t.TempDir()
		homeDir := t.TempDir() // empty — no backend configs

		t.Setenv("XDG_CONFIG_HOME", "")
		t.Setenv("HOME", homeDir)

		gotPath, gotSource, resolveErr := ResolveBackendsConfig(projectDir)
		if !errors.Is(resolveErr, ErrBackendsConfigNotFound) {
			t.Fatalf("expected ErrBackendsConfigNotFound, got path=%q source=%q err=%v",
		...
		// regression.
		if strings.Contains(resolveErr.Error(), "sand/backends.toml") &&
	...
	t.Run("not_found_when_all_rungs_absent", func(t *testing.T) {
		...
		want := []string{
			filepath.Join(projectDir, ".claude", "sand-backends.toml"),
			filepath.Join(xdgDir, "sand", "backends.toml"),
			filepath.Join(homeDir, ".config", "sand", "backends.toml"),
			filepath.Join(homeDir, ".sand", "backends.toml"),
		}
		msg := err.Error()
		for _, p := range want {
...
// ResolveBackendsConfig's return value directly.
