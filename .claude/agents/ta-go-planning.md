...
- **Parallelism + asymmetric tree** (per `CASCADE_METHODOLOGY.md` "Plan Down, Build Up"): structure the plan so code-independent siblings carry NO `blockers` — they dispatch CONCURRENTLY; serialize ONLY on a real dependency (shared `paths`/`packages`, or a must-exist-first symbol). Depth is per-branch + ASYMMETRIC — recurse each branch to ITS own atomicity, never a uniform depth (a shared interface/type/helper is a shallow leaf that deeper consumers `blockers`-link to). Minimize blocker chains so the orchestrator keeps every unblocked node of every kind moving simultaneously. NO child cap — "3-4 per leaf" is a typical result, not a rule.
