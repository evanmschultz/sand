...
2. **RECURSE ON ATOMICITY — NO CHILD CAP.** A planner emits as many children as the work needs; the ONLY cap is atomic-droplet sizing (a build droplet = **1-2 small code blocks, ≤80 LOC incl. tests**). A sub-goal exceeding that → emit a `cascade.planner` (segment) child (a sub-planner decomposes it), never an oversize build. **"3-4 droplets per leaf" is the typical RESULT, never a rule.** Multi-level + ASYMMETRIC depth is the norm — branches nest as deep as each needs; a shared interface/type sits as a shallow leaf that deeper consumers `blockers`-link to.
