...
2. **RECURSE ON ATOMICITY — NO CHILD CAP.** A planner emits as many children as the work needs; the ONLY cap is atomic-droplet sizing (a build droplet = **1-2 small code blocks, ≤80 LOC incl. tests**). A sub-goal exceeding that → emit a `cascade.planner` (segment) child (a sub-planner decomposes it), never an oversize build. **"3-4 droplets per leaf" is the typical RESULT, never a rule.** Multi-level + ASYMMETRIC depth is the norm — branches nest as deep as each needs; a shared interface/type sits as a shallow leaf that deeper consumers `blockers`-link to. **A code block is COUNTABLE — one new/changed top-level production symbol (type/function/method) OR one cohesive same-purpose edit cluster; a new type + a new helper + a different-function rewrite are SEPARATE blocks.** A droplet naming ≥3 distinct production symbols (tests excluded), or >80 LOC, or >3 production files, is OVER BUDGET → sub-plan, never an oversize droplet. **Plan-QA-falsification MEASURES this per droplet and FAILS the plan on any over-budget droplet — never accepting the planner's label — and re-measures EVERY droplet on any plan amendment** (`CASCADE_METHODOLOGY.md` §"Plan Down, Build Up"). **"One coherent concern" / "a single non-separable unit" is NOT an exception to the budget** — a droplet adding a new symbol plus its full test suite is almost always over-budget and splits (twice-observed failure: drop_014, drop_018-D4).
...
**Record id convention reminder (HARD):** every cascade record id is EXACTLY `drop_NNN.drop.<flat_slug>` — three segments, `<flat_slug>` a single `[a-z0-9_]+` token, NO nesting dots. Parent linkage = the `parent_id` field + `blockers[]`, never a dotted child id. (Malformed nested ids become orphan tables unreachable by `mcp__ta__get`.)
